...
def _long2bytes(n, blocksize=0):
   ...
   # print(blocksize)
    ...
    #print(type((blocksize - len(s) % blocksize)* '\000'))
    #print(type(s))
...
def _bytelist2long(list):
    ...
    #print(type(list[9]))
    while i < imax:
        #print((list[j]),(list[j+1]),(list[j+2]),(list[j+3]))
        #print(type(list[3]))
        b0 = (int(ord(list[j])))
        #print(1)
...
class MD5:
    ...
    def __init__(self, md5string='', l=0):
        "Initialisation."
    ...
    def init2(self):
        ...
        self.A = struct.unpack("I",self.md5str[0:8].decode('hex')) [0]
        self.B = struct.unpack("I",self.md5str[8:16].decode('hex')) [0]
        self.C = struct.unpack("I",self.md5str[16:24].decode('hex')) [0]
        self.D = struct.unpack("I",self.md5str[24:32].decode('hex')) [0]
    ...
    def digest(self):
        ...
        count[0]+= self.origin_length*8 #convert to number of bit
